...
if [ -n "$TOKEN_LOCATION" ] && test -f $TOKEN_LOCATION; then
  add_cmd_arg "-v ${TOKEN_LOCATION}:/etc/condor/tokens-orig.d/flock.opensciencegrid.org"
else
